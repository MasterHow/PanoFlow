...
from opticalflow.api.data_augment import distort_flow, distort_img
...
class FlowAugmentor:
    ...
    def __init__(self,
        ...
        # distortion augmentation params
        self.do_distort = do_distort
        self.do_distort_prob = 0.5
        self.k2_limit = 1e-6
        self.k4_limit = 1e-14
        self.k6_limit = 0
    ...
    def distortion_transform(self, img1, img2, flow):
        """Optical distortion augmentation."""

        if self.do_distort:
            if np.random.rand() < self.do_distort_prob:
                # random k
                k2 = random.uniform(-self.k2_limit, self.k2_limit)
                k4 = random.uniform(-self.k4_limit, self.k4_limit)
                k6 = random.uniform(-self.k6_limit, self.k6_limit)

                # distort img and flow
                img1 = distort_img(img1, 'radial', ks=[0, k2, 0, k4, 0, k6])
                img2 = distort_img(img2, 'radial', ks=[0, k2, 0, k4, 0, k6])
                flow = distort_flow(flow, 'radial', ks=[0, k2, 0, k4, 0, k6])
    ...
    def __call__(self, img1, img2, flow):
        ...
        img1, img2, flow = self.distortion_transform(img1, img2, flow)
